...
- 录波：`WaveformCatalog` 固化 18 个读取块；`WaveformDataService` 先读取 1552 框架等级，再负责有符号解码、AD→A 标定、时间轴对齐和 RMS；采样流与普通寄存器轮询共用 `IModbusRtuClient` 的请求锁。
...
录波页先读取 1552（1 次），从 bit8～bit11 得到框架等级和 Rate，再严格按时间段和 A/B/C 相顺序发起 18 次录波读取。1552 失败、框架等级不是 0/1/2 或任一录波块最终失败，都不构造新的 `WaveformData`；ViewModel 只在 19 次请求完整成功后原子替换曲线，因此上一次完整结果不会与新批次混合。主图和主 Excel 使用安培，模型、Shift+F8 明细与明细专用 Excel 保留原始 uint16、有符号 AD 和源地址。
