...
模拟器还提供 B000H～B5BFH 中协议定义的 18 个录波块：每块 64 点，三相为确定性的 50 Hz 波形，故障点后包含幅值变化和衰减分量；负值按 `short` 补码保存，线路仍按 Modbus 大端输出。未定义的地址间隙不填充，读取间隙会返回 02H 非法地址，便于发现块地址或读取数量错误。
